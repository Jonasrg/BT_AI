...
def get_keywords(config:dict)->dict:
    ...
    """
    to_replace = ["(", ")", ", ", ". ", "? ", " of ", " and ", "/", " to ", " in ", " other ", "; ", " for ", " - ", " as ", " own ", " use ", "n.e.c.", "  "]
...
def construct_query(config:dict, save_to_path: str | bool='data/' )->dict:
    ...
    industry_keyword_strings = create_industry_string(industry_keywords=industry_keywords)
    keyword_strings = create_keyword_strings(config=config, industry_keyword_strings=industry_keyword_strings)
...
def create_keyword_strings(config:dict, industry_keyword_strings:dict)->dict:
    ...
    for key in keywords.keys():
        ...
        industry_keyword_len = len(" ".join(industry_keywords[Div_Ind_dict[key]]).split(" "))

        # max 10 keywords per identifier
        step_identifier = 10 - industry_keyword_strings[Div_Ind_dict[key]].count("ta ALL")
...
def create_industry_string(industry_keywords)->dict:
    ...
    for i in industry_keywords.keys():
        industry_string[i] = '(' + 'ta ALL ' + ' OR ta ALL '.join(f'"{j}"' for j in industry_keywords[i]) + ')'
