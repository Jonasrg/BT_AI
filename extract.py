...
def extract_search(response, country, industry, division, query, range_begin, range_end) -> list:
...
def extract_biblio(json_object)->list:
    ...
    for li in json_object:
        documents = li["response"]["ops:world-patent-data"]["ops:biblio-search"]["ops:search-result"]["exchange-documents"]
        ...
        for i in documents:
            ...
            data["document_date"] = element["bibliographic-data"]["publication-reference"]["document-id"][0]["date"]["$"]
            
            # get list of classifications for each patent
            if isinstance(element["bibliographic-data"]["patent-classifications"]["patent-classification"], list):
                data["patent_classifications"] = [{i["classification-scheme"]["@office"]:
                                            [i["section"]["$"],
                                            i["class"]["$"],
                                            i["subclass"]["$"],
                                            i["main-group"]["$"],
                ...
                i = element["bibliographic-data"]["patent-classifications"]["patent-classification"]
                data["patent_classifications"] = [{i["classification-scheme"]["@office"]:
                                            [i["section"]["$"],
                                            i["class"]["$"],
                                            i["subclass"]["$"],
            ...
            data["application-reference"] =  element["bibliographic-data"]["application-reference"]
            data["priority-claims"] =  element["bibliographic-data"]["priority-claims"]

            # get applicants in original format (not in epodoc format)
            data["applicants"] =  [i["applicant-name"]["name"]["$"] for i in element["bibliographic-data"]["parties"]["applicants"]["applicant"] if i["@data-format"] == "original"]
            ...
            if "inventors" in element["bibliographic-data"]["parties"].keys():
                data["inventors"] = [i["inventor-name"]["name"]["$"] for i in element["bibliographic-data"]["parties"]["inventors"]["inventor"] if i["@data-format"] == "original"]
            else:
                data["inventors"] = None
            data["invention-title"] = [{i["@lang"] : i["$"] } for i in element["bibliographic-data"]["invention-title"] if i["@lang"] == "en" or i["@lang"] == "de"]
            
            # check if citation exists
            if "references-cited" in element["bibliographic-data"].keys():
                # check for single citation
                if isinstance(element["bibliographic-data"]["references-cited"]["citation"], dict):
                    element["bibliographic-data"]["references-cited"]["citation"] = [element["bibliographic-data"]["references-cited"]["citation"]]
                # create list of citations with tuples (document-id, name, date); if name and date are unavailable:  name/date = ""
                data["references_cited"] = [ (j["doc-number"]["$"], 
                                              (j["name"]["$"] if "name" in j.keys() else ""),
                                              (j["date"]["$"] if "date" in j.keys() else "") )
                                            for i in element["bibliographic-data"]["references-cited"]["citation"] if "patcit" in i.keys() #patcit = patent citation, sometimes there are other citations
                                            for j in i["patcit"]["document-id"] if j["@document-id-type"] == "epodoc" ]
